...
const DraggableSquares = ({initialSquares}) => {
  ...
  const aiGen = (initialSquares) => {    
    // Create objects for features
    initialSquares.features.forEach(feature => {
        window[feature.name] = {
            x: feature.x,
            y: feature.y,
            width: feature.width,
    ...
    initialSquares.items.forEach(item => {
        window[item.name] = {
            x: item.x,
            y: item.y,
            width: item.width,
    ...
    >
      ...
      <div ref={editorRef} className="fixed bottom-4 right-4 flex flex-row items-end">
        ...
        <div className='space-y-4'>
          <button 
            className="flex items-center justify-center w-12 h-12 bg-white rounded-full shadow-lg hover:bg-gray-50 text-black"
            onClick={aiGen}
          >
            AI </button>
          <button
